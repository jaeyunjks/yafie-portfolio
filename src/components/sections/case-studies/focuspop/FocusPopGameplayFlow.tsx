...
import { focusPopGameplaySteps } from "@/data/caseStudies/focuspop";
import { FocusPopSectionLabel } from "./FocusPopVisuals";

export default function FocusPopGameplayFlow() {
  const [activeStep, setActiveStep] = useState(0);
  const selected = focusPopGameplaySteps[activeStep];

  return (
    <section id="gameplay-flow" className="scroll-mt-32 px-6 py-12">
      <div className="mx-auto max-w-7xl">
        <Reveal>
          <FocusPopSectionLabel
            label="01 // gameplay.flow"
            title="How the game works."
            subtitle="A simple loop from setup to score review."
          />
        </Reveal>
        ...
        <Reveal delay={0.06}>
          <div className="open-panel mt-8">
            <div
              ...
              role="group"
              aria-label="FocusPop gameplay flow steps"
            >
              {focusPopGameplaySteps.map((step, index) => {
