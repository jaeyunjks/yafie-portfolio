...
const mobileFilters = [
  ...
  { label: "Framework", value: "Frameworks" },
  { label: "Tech Tools", value: "Tech & Developer Tools" },
];
...
export default function AboutTechStack() {
  const [activeFilter, setActiveFilter] = useState(mobileFilters[0].value);
  const activeCategory =
    techStack.find((category) => category.title === activeFilter) ?? techStack[0];
  const activeIndex = techStack.findIndex(
    (category) => category.title === activeCategory.title,
  );
    ...
    <section className="pb-20 lg:pb-24">
        ...
        {mobileFilters.map((filter) => {
          ...
          return (
            <button
              key={filter.value}
              type="button"
              className={`shrink-0 rounded-full border px-4 py-2 text-xs font-extrabold transition-[background-color,border-color,color,box-shadow] duration-200 ${
                isActive
                  ? "border-[#8dbbff]/70 bg-[#2d5f9d] text-white shadow-[0_12px_30px_rgba(45,95,157,0.16)]"
                  : "border-[#d4e3ff]/72 bg-white/66 text-slate-600 shadow-sm"
              }`}
      ...
      <div className="md:hidden">
        <Reveal key={activeCategory.title}>
          <TechStackCard category={activeCategory} index={activeIndex} />
        </Reveal>
      </div>
      ...
      <div className="hidden gap-4 md:grid md:grid-cols-2 xl:grid-cols-3">
        {techStack.map((category, index) => (
          <Reveal key={category.title} delay={index * 0.06}>
            <TechStackCard category={category} index={index} />
          </Reveal>
        ))}
      </div>
    </section>
